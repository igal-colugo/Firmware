 ...
 */
void encodePMU_PumpOptionBits_t(uint8_t *_pg_data, int *_pg_bytecount, const PMU_PumpOptionBits_t *_pg_user)
{
 ...
 */
int decodePMU_PumpOptionBits_t(const uint8_t *_pg_data, int *_pg_bytecount, PMU_PumpOptionBits_t *_pg_user)
{
    int _pg_byteindex = *_pg_bytecount;
 ...
 */
void encodePMU_ThrottleSettingsPacketStructure(void *_pg_pkt, const PMU_ThrottleSettings_t *_pg_user)
{
    ...
    encodePMU_ThrottleCurveConfigBits_t(_pg_data, &_pg_byteindex, &_pg_user->curveConfig);
    ...
    finishPMUPacket(_pg_pkt, _pg_byteindex, getPMU_ThrottleSettingsPacketID());
 ...
 */
int decodePMU_ThrottleSettingsPacketStructure(const void *_pg_pkt, PMU_ThrottleSettings_t *_pg_user)
{
    int _pg_numbytes;
    int _pg_byteindex = 0;
    ...
    if (getPMUPacketID(_pg_pkt) != getPMU_ThrottleSettingsPacketID())
        return 0;

    // Verify the packet size
    _pg_numbytes = getPMUPacketSize(_pg_pkt);
    if (_pg_numbytes < getPMU_ThrottleSettingsMinDataLength())
        return 0;
    ...
    // Throttle curve config bits
    if (decodePMU_ThrottleCurveConfigBits_t(_pg_data, &_pg_byteindex, &_pg_user->curveConfig) == 0)
        return 0;
 ...
 * \param _pg_user points to the user data that will be encoded in _pg_pkt
 */
void encodePMU_FuelUsedSettingsPacketStructure(void *_pg_pkt, const PMU_FuelUsedSettings_t *_pg_user)
{
    uint8_t *_pg_data = getPMUPacketData(_pg_pkt);
    int _pg_byteindex = 0;
    ...
    // complete the process of creating the packet
    finishPMUPacket(_pg_pkt, _pg_byteindex, getPMU_FuelUsedSettingsPacketID());
 ...
 * \return 0 is returned if the packet ID or size is wrong, else 1
 */
int decodePMU_FuelUsedSettingsPacketStructure(const void *_pg_pkt, PMU_FuelUsedSettings_t *_pg_user)
{
    int _pg_numbytes;
    int _pg_byteindex = 0;
    const uint8_t* _pg_data;
    ...
    // Verify the packet identifier
    if (getPMUPacketID(_pg_pkt) != getPMU_FuelUsedSettingsPacketID())
        return 0;

    // Verify the packet size
    _pg_numbytes = getPMUPacketSize(_pg_pkt);
    if (_pg_numbytes < getPMU_FuelUsedSettingsMinDataLength())
        return 0;
 ...
 * \param _pg_user points to the user data that will be encoded in _pg_pkt
 */
void encodePMU_GovernorSettingsPacketStructure(void *_pg_pkt, const PMU_GovernorSettings_t *_pg_user)
{
    uint8_t *_pg_data = getPMUPacketData(_pg_pkt);
    int _pg_byteindex = 0;
    ...
    // complete the process of creating the packet
    finishPMUPacket(_pg_pkt, _pg_byteindex, getPMU_GovernorSettingsPacketID());
 ...
 * \return 0 is returned if the packet ID or size is wrong, else 1
 */
int decodePMU_GovernorSettingsPacketStructure(const void *_pg_pkt, PMU_GovernorSettings_t *_pg_user)
{
    int _pg_numbytes;
    int _pg_byteindex = 0;
    const uint8_t* _pg_data;
    ...
    // Verify the packet identifier
    if (getPMUPacketID(_pg_pkt) != getPMU_GovernorSettingsPacketID())
        return 0;

    // Verify the packet size
    _pg_numbytes = getPMUPacketSize(_pg_pkt);
    if (_pg_numbytes < getPMU_GovernorSettingsMinDataLength())
        return 0;
 ...
 * \param _pg_user points to the user data that will be encoded in _pg_pkt
 */
void encodePMU_PumpSettingsPacketStructure(void *_pg_pkt, const PMU_PumpSettings_t *_pg_user)
{
    uint8_t *_pg_data = getPMUPacketData(_pg_pkt);
    int _pg_byteindex = 0;
    ...
    encodePMU_PumpOptionBits_t(_pg_data, &_pg_byteindex, &_pg_user->options);
    ...
    // complete the process of creating the packet
    finishPMUPacket(_pg_pkt, _pg_byteindex, getPMU_PumpSettingsPacketID());
 ...
 * \return 0 is returned if the packet ID or size is wrong, else 1
 */
int decodePMU_PumpSettingsPacketStructure(const void *_pg_pkt, PMU_PumpSettings_t *_pg_user)
{
    int _pg_numbytes;
    int _pg_byteindex = 0;
    const uint8_t* _pg_data;
    ...
    // Verify the packet identifier
    if (getPMUPacketID(_pg_pkt) != getPMU_PumpSettingsPacketID())
        return 0;

    // Verify the packet size
    _pg_numbytes = getPMUPacketSize(_pg_pkt);
    if (_pg_numbytes < getPMU_PumpSettingsMinDataLength())
        return 0;
    ...
    // Pump control system options
    if (decodePMU_PumpOptionBits_t(_pg_data, &_pg_byteindex, &_pg_user->options) == 0)
        return 0;
 ...
 * \param _pg_user points to the user data that will be encoded in _pg_pkt
 */
void encodePMU_ECUDataPacketStructure(void *_pg_pkt, const PMU_PMUData_t *_pg_user)
{
    uint8_t *_pg_data = getPMUPacketData(_pg_pkt);
    int _pg_byteindex = 0;
    unsigned _pg_i = 0;
    ...
    // complete the process of creating the packet
    finishPMUPacket(_pg_pkt, _pg_byteindex, getPMU_PMUDataPacketID());
 ...
 * \return 0 is returned if the packet ID or size is wrong, else 1
 */
int decodePMU_ECUDataPacketStructure(const void *_pg_pkt, PMU_PMUData_t *_pg_user)
{
    int _pg_numbytes;
    int _pg_byteindex = 0;
    const uint8_t* _pg_data;
    ...
    // Verify the packet identifier
    if (getPMUPacketID(_pg_pkt) != getPMU_PMUDataPacketID())
        return 0;

    // Verify the packet size
    _pg_numbytes = getPMUPacketSize(_pg_pkt);
    if (_pg_numbytes < getPMU_PMUDataMinDataLength())
        return 0;
